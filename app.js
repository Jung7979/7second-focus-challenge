...
let selectedSound = 'rain', startedAt = 0, audioContext, noiseSource, muted = false;
...
document.querySelector('#start-button').addEventListener('click', () => {
  ...
  show('play'); startedAt = performance.now(); noiseSource = createNoise(selectedSound);
});

document.querySelector('#stop-button').addEventListener('click', () => {
  const elapsed = (performance.now() - startedAt) / 1000; const difference = Math.abs(elapsed - 7);
  stopSound(); document.querySelector('#record-value').textContent = `${elapsed.toFixed(2)}초`;
...
document.querySelector('#retry-button').addEventListener('click', () => { stopSound(); show('intro'); });
